var ChatApp = window.React.createClass({
    ...
    render: function(){
        ...
        var messages = self.state.messages.map(function (msg) {
            return <li><strong>{msg.body}</strong>&nbsp;&nbsp;<span>{msg.user}</span></li>
        });
        return(
            <div>
                <ul>
                ...
                <input id="user" type="text" placeholder="choose a username"/><button onClick={ ()=>self.pickUser() }>Pick Username</button>
            </div>
        )
    }
});
